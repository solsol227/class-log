...
-- These trigger functions call internal helpers whose EXECUTE privilege is
-- intentionally not exposed to authenticated clients. Run only the trigger
-- body with the function owner's privileges and retain the fixed search_path.
alter function public.validate_assignment_schedule() security definer;
alter function public.validate_lesson_schedule_change() security definer;
