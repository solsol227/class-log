...
const PUBLICATIONS = new Set(["all", "published", "unpublished"]);
const RANGE_OPTIONS = [
  { value: "all", label: "전체" },
...
type PageProps = {
  ...
  searchParams: Promise<{ range?: string; from?: string; to?: string; sort?: string; publication?: string }>;
};
...
export default async function OperatorStudentFeedbackPage({ params, searchParams }: PageProps) {
  ...
  const publication = PUBLICATIONS.has(rawQuery.publication ?? "all") ? rawQuery.publication ?? "all" : "all";
  const publicationError = rawQuery.publication && !PUBLICATIONS.has(rawQuery.publication)
    ? "게시 상태 조건이 올바르지 않아 전체 상태를 표시합니다."
    : null;
  ...
  const allFeedback = await loadOperatorStudentFeedback(supabase, studentResult.data);
  const filtered = allFeedback.filter((item) => {
    ...
    if (publication === "published" && !item.publishedAt) return false;
    if (publication === "unpublished" && item.publishedAt) return false;
    return true;
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-5 py-10 sm:px-8 sm:py-14">
      ...
      <section className="mt-7 rounded-2xl border border-[var(--line)] bg-white p-4 sm:p-5" aria-labelledby="operator-feedback-filters">
        <h2 id="operator-feedback-filters" className="text-lg font-bold">기간 · 정렬 · 게시 상태</h2>
        ...
        <form method="get" className="mt-4 grid gap-3 sm:grid-cols-[1fr_1fr_auto] sm:items-end">
          ...
          <label className="grid gap-2 text-sm font-bold">게시 상태<select name="publication" defaultValue={publication} className="min-h-11 rounded-xl border border-[var(--line)] bg-white px-3 font-normal"><option value="all">전체</option><option value="published">게시</option><option value="unpublished">미게시</option></select></label>
          <button className="min-h-11 rounded-xl bg-[var(--foreground)] px-4 font-bold text-white">필터 적용</button>
        </form>
      </section>

      {parsed.error || publicationError ? <p role="alert" className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 font-semibold text-amber-900">{parsed.error ?? publicationError}</p> : null}
