...
export function StudentProfileCard({ student, programs }: { student: StudentProfile; programs: StudentProgram[] }) {
  ...
  return <section className="rounded-2xl border border-[var(--line)] bg-white p-6 shadow-[0_24px_70px_rgba(23,64,60,0.09)] sm:p-8">
    ...
    {editing ? <StudentEditForm student={student} onCancel={() => { setEditing(false); setProfileNotice("학생 기본정보 편집을 취소했습니다."); }} onSaved={() => { setEditing(false); setProfileNotice("학생 기본정보를 저장했습니다."); }} /> : <StudentProfileView student={student} onEdit={() => { setEditing(true); setProfileNotice(""); }} />}
    <section className="mt-8 border-t border-[var(--line)] pt-6">
      {programNotice ? <p role="status" className="mb-4 text-sm font-bold text-[var(--accent-strong)]">{programNotice}</p> : null}
      {editingPrograms ? <StudentProgramsForm studentId={student.id} programs={programs} onCancel={() => { setEditingPrograms(false); setProgramNotice("이용프로그램 편집을 취소했습니다."); }} onSaved={() => { setEditingPrograms(false); setProgramNotice("이용프로그램을 저장했습니다."); }} /> : <><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-bold">이용프로그램</h2><button type="button" onClick={() => { setEditingPrograms(true); setProgramNotice(""); }} className="min-h-11 rounded-xl border border-[var(--line)] px-4 font-bold">프로그램 관리</button></div><ProgramHistory programs={programs} /></>}
    </section>
...
function StudentProfileView({ student, onEdit }: { student: StudentProfile; onEdit: () => void }) {
