...
export function StudentProfileCard({ student, programs, canManage }: { student: StudentProfile; programs: StudentProgram[]; canManage: boolean }) {
  ...
  return <section className="rounded-2xl border border-[var(--line)] bg-white p-6 shadow-[0_24px_70px_rgba(23,64,60,0.09)] sm:p-8">
    ...
    {editing && canManage ? <StudentEditForm student={student} onCancel={() => { setEditing(false); setProfileNotice("학생 기본정보 편집을 취소했습니다."); }} onSaved={() => { setEditing(false); setProfileNotice("학생 기본정보를 저장했습니다."); }} /> : <StudentProfileView student={student} canManage={canManage} onEdit={() => { setEditing(true); setProfileNotice(""); }} />}
    <section className="mt-8 border-t border-[var(--line)] pt-6">
      {programNotice ? <p role="status" className="mb-4 text-sm font-bold text-[var(--accent-strong)]">{programNotice}</p> : null}
      {editingPrograms && canManage ? <StudentProgramsForm studentId={student.id} programs={programs} onCancel={() => { setEditingPrograms(false); setProgramNotice("이용프로그램 편집을 취소했습니다."); }} onSaved={() => { setEditingPrograms(false); setProgramNotice("이용프로그램을 저장했습니다."); }} /> : <><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-bold">이용프로그램</h2>{canManage ? <button type="button" onClick={() => { setEditingPrograms(true); setProgramNotice(""); }} className="min-h-11 rounded-xl border border-[var(--line)] px-4 font-bold">프로그램 관리</button> : null}</div><ProgramHistory programs={programs} /></>}
    </section>
    {canManage ? <div className="mt-8 border-t border-[var(--line)] pt-6"><form action={deleteAction} onSubmit={(event) => { if (!window.confirm("이 학생을 삭제하시겠습니까? 학생 계정과 배정된 일정 및 관련 데이터가 함께 삭제될 수 있습니다.")) event.preventDefault(); }}>{deleteState.formError ? <p role="alert" className="mb-4 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-bold text-rose-900">{deleteState.formError}</p> : null}<DeleteButton /></form></div> : null}
  </section>;
}

function StudentProfileView({ student, canManage, onEdit }: { student: StudentProfile; canManage: boolean; onEdit: () => void }) {
  return <><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-bold tracking-[0.12em] text-[var(--accent-strong)]">학생 정보</p><h1 className="mt-3 text-3xl font-bold tracking-[-0.04em] sm:text-4xl">{student.name}</h1></div>{canManage ? <button type="button" onClick={onEdit} className="min-h-10 rounded-xl border border-[var(--line)] px-4 font-bold transition hover:border-[var(--accent)] hover:text-[var(--accent-strong)]">수정</button> : null}</div>
    <dl className="mt-8 grid gap-x-8 gap-y-5 sm:grid-cols-2"><ProfileValue label="성별" value={student.gender === "male" ? "남" : student.gender === "female" ? "여" : "-"} /><ProfileValue label="나이" value={student.age?.toString() ?? "-"} /><ProfileValue label="연락처" value={student.phone ? formatPhone(student.phone) : "-"} /><ProfileValue label="유입경로" value={student.acquisitionSource ? ({ instagram: "인스타그램", daangn: "당근", referral: "지인소개", naver: "네이버" }[student.acquisitionSource] ?? student.acquisitionSource) : "-"} /><ProfileValue label="유입시기" value={formatJoinedMonth(student.joinedMonth)} /><div className="sm:col-span-2"><dt className="text-sm font-bold text-[var(--muted)]">특이사항</dt><dd className="mt-1 whitespace-pre-wrap leading-7">{student.specialNotes ?? "-"}</dd></div></dl></>;
}
