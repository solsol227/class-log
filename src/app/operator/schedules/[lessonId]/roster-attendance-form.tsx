...
import { StudentFeedbackModal, type FeedbackAuthorOption, type StudentFeedbackModalData } from "./student-feedback-modal";
...
export function RosterAttendanceForm({ lessonId, students, blockedReason, feedbackByStudent, feedbackAuthorOptions, feedbackBlockedReason, isOwner }: { lessonId: string; students: RosterStudent[]; blockedReason: string | null; feedbackByStudent: StudentFeedbackModalData[]; feedbackAuthorOptions: FeedbackAuthorOption[]; feedbackBlockedReason: string | null; isOwner: boolean }) {
  const [state, formAction] = useActionState(saveRosterAttendance.bind(null, lessonId), INITIAL_STATE);
  const [selectedStudentId, setSelectedStudentId] = useState<string | null>(null);
  const triggerRef = useRef<HTMLButtonElement | null>(null);
  const selectedFeedback = feedbackByStudent.find((entry) => entry.studentId === selectedStudentId) ?? null;
  ...
  return (
        ...
        {students.map((student) => {
          ...
          const makeupNotice = student.makeupStatus === "completed"
            ...
            : student.status === "excused" && student.makeupStatus === "scheduled"
              ...
              : null;
          return (
            <fieldset key={student.id} className="grid gap-3 rounded-xl border border-[var(--line)] p-4 sm:grid-cols-[minmax(9rem,1fr)_minmax(0,2fr)] sm:items-center">
              <legend className="sr-only">{student.name} 출결 상태</legend>
              <div>
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="font-bold">{student.name}</p>
                  <button type="button" onClick={(event) => { triggerRef.current = event.currentTarget; setSelectedStudentId(student.id); }} className="inline-flex min-h-10 items-center rounded-lg border border-[var(--accent)] px-3 text-sm font-bold text-[var(--accent-strong)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--accent)]">피드백 작성</button>
                </div>
                ...
                {OPTIONS.map((option) => (
                  <label key={option.value} className="flex min-h-11 cursor-pointer items-center justify-center rounded-xl border border-[#9badaa] bg-white px-2 text-center text-sm font-bold has-checked:border-[var(--accent)] has-checked:bg-[#e5f2f0] has-checked:text-[var(--accent-strong)] has-disabled:cursor-not-allowed has-disabled:opacity-60">
                    <input
                      ...
                      disabled={Boolean(blockedReason) || (makeupLocksAttendance && option.value !== "excused")}
                      className="sr-only"
                    />
                    {option.label}
                  </label>
    ...
    {selectedFeedback ? <StudentFeedbackModal key={selectedFeedback.studentId} lessonId={lessonId} data={selectedFeedback} authorOptions={feedbackAuthorOptions} blockedReason={feedbackBlockedReason} isOwner={isOwner} onClosed={() => { setSelectedStudentId(null); triggerRef.current?.focus(); }} /> : null}
    </>
  );
}
