...
export function OperatorFeedbackList({ items, emptyMessage }: { items: OperatorFeedbackItem[]; emptyMessage: string }) {
  ...
  return <>
    <ol className="mt-5 space-y-3">{items.map((item) => <li key={item.id}><article className="rounded-xl border border-[var(--line)] p-4 sm:p-5"><div className="flex flex-wrap items-center gap-2 text-sm"><time className="font-bold text-[var(--accent-strong)]" dateTime={item.startsAt}>{formatDate(item.startsAt)}</time><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${item.publishedAt ? "bg-emerald-100 text-emerald-900" : "bg-amber-100 text-amber-900"}`}>{item.publishedAt ? "게시" : "미게시"}</span></div><h3 className="mt-3 truncate text-lg font-bold">{item.lessonTitle}</h3><p className="mt-2 truncate">{item.body}</p><p className="mt-2 text-sm text-[var(--muted)]">{item.authorName} · 댓글 {item.commentCount}개</p><button type="button" onClick={(event) => { triggerRef.current = event.currentTarget; setSelected(item); }} className="mt-4 min-h-11 rounded-xl border border-[var(--accent)] px-4 font-bold text-[var(--accent-strong)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--accent)]">더보기</button></article></li>)}</ol>
