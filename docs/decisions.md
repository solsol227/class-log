...
실제 정상 미게시 피드백이 0건임을 확인한 뒤 `lesson_feedback.published_at`과 게시 상태 UI·조건을 제거한다. 모든 정상 피드백은 작성 즉시 학생에게 보이는 단일 상태이며, 학생 RLS는 본인 active assignment, 비-Draft 일정, 미삭제 피드백 조건만 사용한다.

사용자 승인에 따라 현재 `deleted_at` 표시 피드백·댓글은 즉시 물리 삭제한다. 이후 삭제는 7일 복구 유예 뒤 DB 내부 pg_cron 작업이 정리한다. 삭제된 피드백의 댓글은 함께 삭제하고, 개별 삭제 댓글 아래 남은 답글은 최상위 댓글로 보존한다. 배정 해제와 프로그램 중단 같은 업무 이력은 삭제 정책 대상이 아니다.

학생 상세의 일정 다중 배정은 owner 전용 atomic RPC 한 번으로 처리하고 batch 전체에 하나의 이용권을 사용한다. 기본정보와 이용프로그램 편집은 분리하며, 이용 횟수 관리는 프로그램 편집 카드 안으로 통합한다.

### 이유

운영 화면에 게시 전환 기능이 없고 정상 미게시 데이터도 없어 의미 없는 상태를 유지할 이유가 없다. 삭제 직후 복구 가능성과 장기 개인정보 최소 보존을 함께 만족시키고, 다중 일정 배정의 부분 성공과 quota 경쟁 조건을 막기 위해서다.

---

## 2026-09-10 — 일정 상세 피드백은 학생별 modal로 통일

### 결정

일정 상세 하단의 통합 피드백 대시보드를 제거하고 roster의 학생별 `피드백 작성` 버튼이 현재 화면 위의 modal을 연다. modal의 신규 피드백은 즉시 게시하며 저장 후에도 modal을 유지한다. 기존 피드백은 전체 본문과 날짜·실제 제공 직원·게시 상태만 읽기 전용으로 보여주고, 댓글/답글은 삭제되지 않은 row의 합계 배지만 표시한다. 댓글 본문과 mutation UI는 기존 상세 경로에 유지한다.

제공 직원은 해당 일정에 배정된 활성 직원으로 제한한다. staff는 본인 담당 일정에서 본인을 자동 사용하고, owner도 일정 담당자 범위 밖의 직원을 선택할 수 없다.

### 이유

출결과 피드백 작성 사이의 화면 전환을 없애면서도 기존 공식 기록과 대화 기능의 책임을 섞지 않고, PR30의 owner/staff 권한 및 실제 제공자 관계를 UI와 서버에서 동일하게 유지하기 위해서다.

### 후속 보완

학생 상세·전체 피드백의 조회 dialog도 별도 수정 page로 이동하지 않고 popup 안에서 본문을 직접 수정한다. 게시 상태·상세 시각 metadata box는 popup에서 반복하지 않으며 날짜·시간·제공자를 한 줄로 표시한다. 로그인한 운영자가 직접 작성한 댓글·답글에만 작은 수정·삭제 control을 제공하고 삭제는 기존 soft-delete 정책을 유지한다.
...
## 2026-09-12 — 학생 목표는 students의 선택 필드로 관리

### 결정

학생 목표는 `students.goal text null`에 최대 1000자로 저장한다. owner는 학생 프로필에서 등록·수정하고 staff는 조회만 한다. student는 내 일정의 이용권 대시보드에서 같은 목표를 조회하고 자기 목표만 수정한다. 빈 목표는 연한 `나만의 목표를 설정해볼까요?` placeholder로 안내한다.

### 이유

목표는 학생 프로필에 직접 속하는 단일 값이며 별도 이력·관계 구조나 운영자/학생 복사본이 필요하지 않다. 조회에는 기존 students RLS를 재사용한다. 학생 수정은 대상 ID를 받지 않는 전용 `update_my_student_goal` RPC로 한정해 다른 프로필 필드의 UPDATE 권한과 타학생 접근을 열지 않는다.

## 2026-09-12 — 목록 잔여 횟수는 기존 allowance read model 재사용

### 결정

학생 목록은 `student_program_allowance_statuses`를 목록 전체에 한 번 조회하고 active enrollment와 결합한다. 신규 집계 RPC나 학생별 query를 만들지 않는다.

### 이유

상세·학생 화면과 같은 allowance 계산을 재사용해 상태 해석이 달라지는 것을 막고, stopped 이력이나 횟수 미설정 프로그램을 임의의 `0회`로 표현하지 않으면서 N+1도 피할 수 있다.
